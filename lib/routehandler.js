...
routeHandler.Lend = (data, callback)=>{
  ...
  if (acceptableHeaders.indexOf(data.method)> -1){
    ...
    helper.validateID(`./.data/books/${bookID}.json`);
    // read the users folder with the userID, ensure that the user exists
    helper.validateID(`./.data/users/${userID}.json`,bookID);
    callback(200, { message: 'user has successfully borrowed a book'});
  }else {
    callback(405);
  }
};
...
routeHandler.ReturnBook = (data, callback)=>{
  ...
  if (acceptableHeaders.indexOf(data.method)> -1){
    ...
    // save the data.payload.id as bookID
    let bookID = data.payload.id;
    // read the books folder with the bookID, 
    helper.returnbook(`./.data/books/${bookID}.json`);
    // read the users folder with the userID
    helper.returnbook(`./.data/users/${userID}.json`,bookID);
    callback(200, { message: 'user has successfully returned a book to the library'});
  }else {
    callback(405);
  }
};
